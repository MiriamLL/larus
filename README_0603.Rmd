...
Path_02locs <- do.call("rbind", trips_list)
```

## calculate leaving and returning distance

```{r,eval=FALSE, echo=FALSE}
calculate_leaving_distance<-function(my_locs=my_locs,my_central_location=my_central_location){
  
  # trips
  my_paths<-my_locs %>%
    dplyr::group_by(trip_number)%>%
    dplyr::summarise(first_lat=first(Latitude),
...
```{r}
LALI05_06jun_reevaluate_01trips<-subset_reevaluation(my_trip=LALI05_06jun_03trips,   
                                                     my_tripid='trip_00231',
                                                     my_central_location=rest_central_location)
```

```{r}
LALI05_06jun_reevaluate_02trips<-identify_trips_reevaluation(my_trip=LALI05_06jun_reevaluate_01trips,
                                                            my_central_location=rest_central_location,
                                                            my_previous_params=LALI05_06jun_04params$trip_id)
```

```{r}
plot_trips(my_locs=LALI05_06jun_reeavluate_02trips,my_central_location=rest_central_location)+
  theme(legend.position='right')
```

```{r}
LALI05_06jun_reeavaluate_05params<-calculate_params(my_locs=LALI05_06jun_reevaluate_02trips,
                                 my_daytime='daytime',
                                 my_format=  "%Y-%m-%d %H:%M:%S",
                                 my_units="hours",
                                 my_divider="trip_number",
                                 my_gaps="gaps_min")
head(LALI05_06jun_reeavaluate_05params)
```

```{r}
LALI05_06jun_reevaluateC_05params<-LALI05_06jun_reeavaluate_05params %>%
  dplyr::mutate(trip_month_id=paste0(This_month_text,"_c_",trip_id))%>%
  dplyr::mutate(central_loc='south_of_colony')
```
