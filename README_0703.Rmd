...
```{r}
Params_path<-Params_leaving_returning %>%
  rename(trip_id=trip_number)%>%
  right_join(Params_path,by='trip_id')
```

Sum distances

```{r}
Params_12path<-Params_11path %>%
  rename(central_location=central_location.y)%>%
  mutate(sum_path_lenght=case_when(central_location == "colony" ~ 
...
```{r}
Params_13path %>%
  arrange(-sum_path_lenght)
```

```{r}
plot_trips(my_locs=Path_01locs %>%
  filter(trip_number=='trip_00060'),my_central_location=nest_central_location)+
  ggtitle('This trip leaving location was far from the colony, meaning previous locations were not recorded')
```

# Check speed

```{r}
Path_05locs<-Path_03distances %>%
  mutate(speed_if_directly_flying=pointsdist_km/gaps_min*60)%>%
  dplyr::select(ID,trip_number,num_seq,
         countday,season,month,
         daytime,
         Longitude,Latitude,
         battery_charge,battery_class,
         gaps_class,
         central_location,day_or_night,
         trip_size,resolution,params_analyses,interpolation,
         pointsdist_km,gaps_min,
         ground.speed,speed_if_directly_flying
         )
```
...
```{r}
plot_trips(my_locs=Path_01locs %>%
  filter(trip_number=='trip_00283'),my_central_location=nest_central_location)+
  ggtitle('This trip leaving location was far from the colony, meaning previous locations were not recorded')
```
and remember gulls do whatever they want

```{r}
Params_14path<-Params_13path %>%
  filter(trip_number!='trip_00283')%>%
  rename(trip_id=trip_number)
Path_06locs<-Path_05locs %>%
  filter(trip_number!='trip_00283')
```
...
```{r}
compare_notrips(my_params=Params_14path,my_locs=Path_06locs)
```
