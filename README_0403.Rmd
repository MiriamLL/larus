...
gaps_params<-function(my_params=my_params,my_locs=my_locs){
  ...
  gaps_params<-my_locs %>%
    ...
    summarise(min_gap=min(gaps_min),
...
Reevaluate_01locs<-Trips_03trips %>%
  ...
  relocate(ID,trip_number,daytime,gaps_min)
...
```{r}
Inter_03locs %>% group_by(trip_number)%>%tally()%>%summarise(last=tail(trip_number))
```
...
```{r}
this_trip<-'trip_00679'
```

```{r}
Inter_05reevaluate<-Inter_03locs %>%
  filter(trip_number==this_trip)
```

#### new central location

```{r}
new_central_location<-data.frame(Longitude=-110.325,Latitude=24.15)
plot_check(my_locs=Inter_05reevaluate,my_central_location = new_central_location)+
  geom_point(data=new_central_location, aes(x=Longitude, y=Latitude),color='red',shape=17, size=5)
```

# reevaluate_params

```{r}
reevaluate_params<-function(this_locs,
...
```{r}
Inter_06params<-reevaluate_params(this_locs=Inter_05reevaluate,
                     this_central_location=new_central_location,
                     this_previous=data.frame(trip_id=c('trip_00679'))
)
Inter_06params
```
...
```{r}
plot_trips(my_locs=Inter_05reevaluate,my_central_location = new_central_location)+
  geom_point(data=new_central_location, aes(x=Longitude, y=Latitude),color='blue',shape=17, size=5)
```
