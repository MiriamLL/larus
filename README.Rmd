...
Making the separation by individual and month, is more time consuming but assures that you can have an overview of the trips and the calculations.

Also the birds might leave the nest at some point and you need to recalculate the central location.
...
Create data frame with trip_start, trip_end, duration, gap minimum, gap maximum per trip
...
Check that the same number of trips are in the locations and in the parameters
...
Add information of the criteria to keep or not some trips based on trip duration (trip_size), resolution (recording intervals), duration (trips should be shorter than 24 hrs), interpolation (just trips that are longer than 30 minutes, have intervals of < 60 minutes and last then than 24 hours would be considered for interpolation)
...
Check if the trips were not longer than 24 hrs (longer_than_24h_reevaluate_centralloc)
...
Some trips might not seem real, is it because of the resolution or because the animal move from central location?
...
Use the plot to change the central location, either by seeing all trips or one trip at a time
...
This function gives the information if the location was in or out of the central location
...
```{r, eval=FALSE, echo=FALSE}
usethis::use_r("subset_reevaluation")
subset_reevaluation<-function(my_tripid=my_tripid,
                              my_trip=my_trip,
                              my_central_location=my_central_location){
  ...
  print(plot_check(my_locs=my_locs,my_central_location = nest_central_location)+
    ggplot2::geom_point(data=my_central_location, ggplot2::aes(x=Longitude, y=Latitude),color='orange',shape=17, size=5)+
...
```{r, eval=FALSE, echo=FALSE}
#Move to folder data
save(Path_trips, file = "Path_trips.RData", version = 2)
#Run in console, not Rmd
usethis::use_r("Path_trips")
devtools::document()
devtools::check()
```

```{r,eval=FALSE, echo=FALSE}
# this function was not included
calculate_distances<-function(my_data=my_data){
  my_coords<- my_data[,c('Longitude','Latitude')]
  my_spatial <- sp::SpatialPointsDataFrame(coords = my_coords, data = my_data)
  sp::proj4string(my_spatial)= sp::CRS("+init=epsg:4326")
  my_distance<-sapply(2:nrow(my_spatial),
                             function(i){geosphere::distm(my_spatial[i-1,], my_spatial[i,])})
  my_distances<-c(NA,my_distance)
  my_data$dist_km<-my_distances/1000
  return(my_data)
}
calculate_distances(my_data=GSM_trips)
```

## Step 2: distances_per_trip

This function is a loop that separates trip per divider and calculate distances per trip and adds them to the locations

```{r,eval=FALSE, echo=FALSE}
usethis::use_r("distances_per_trip")

distances_per_trip<-function(my_df=my_df,
                             my_divider=my_divider){
  trips_df<-my_df
  trips_df$separator <- (trips_df[['trip_number']])
...
Be careful, to calculate path distances you must have at least three locations per trip
Takes time
...
```{r,eval=FALSE, echo=FALSE}
usethis::use_r("calculate_leaving_returning")
...
#'
#' @return
#' @export returns a data frame with the trip number, the distance of leaving and returning, and the coordinates of the central location
#'
...
Careful: exclude those trips that the central_location was south of the colony for the corrections.
...
The part of don't interpolate would be large because it includes resting and overnight locations
...
Trip 0512 Seem legit, and maybe should be reconsidered if central location is identified. 
